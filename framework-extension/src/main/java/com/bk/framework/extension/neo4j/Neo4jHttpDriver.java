 ...
 * @author BK
 * @version V2.0
 * @description: neo4j http driver
 * @date 2019/4/30 9:38
 */
public final class Neo4jHttpDriver extends AbstractConfigurableDriver {
    private static final Logger LOGGER = LoggerFactory.getLogger(Neo4jHttpDriver.class);
    ...
    public Request request() {
        ...
        if (tx == null) {
            return new Neo4jHttpRequestAdaptor(new HttpRequest(httpClient(), requestUrl(), configuration.getCredentials()));
        } else {
            return new Neo4jHttpRequestAdaptor(new HttpRequest(httpClient(), requestUrl(), configuration.getCredentials(), tx.isReadOnly()));
        }
    }
